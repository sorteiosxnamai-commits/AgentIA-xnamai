...
_COOKIE_CATEGORIAS_SESSAO = "mercos_categorias_sessao"
_COOKIE_TABELAS_PRECO_CURSOR = "mercos_tabelas_preco_cursor"
_COOKIE_TABELAS_PRECO_SESSAO = "mercos_tabelas_preco_sessao"
...
def _cursor_categorias(request: Request, cursor_form: str = "") -> str:
    form = (cursor_form or "").strip()
    if form:
        return form
    raw = (request.cookies.get(_COOKIE_CATEGORIAS_CURSOR) or "").strip().strip('"')
    if not raw:
        return ""
    try:
        return unquote(raw)
...
def _set_cursor_categorias_cookie(resp: HTMLResponse, cursor: str | None) -> HTMLResponse:
    valor = (cursor or "").strip()
    if valor:
        resp.set_cookie(
            key=_COOKIE_CATEGORIAS_CURSOR,
            value=quote(valor, safe=""),
            httponly=False,
            max_age=_CURSOR_MAX_AGE,
            samesite="lax",
        ...
        resp.delete_cookie(_COOKIE_CATEGORIAS_CURSOR)
    return resp


def _obter_ou_criar_sessao_categorias(request: Request) -> str:
    existente = (request.cookies.get(_COOKIE_CATEGORIAS_SESSAO) or "").strip()
    if existente:
        return existente
    return uuid.uuid4().hex


def _garantir_sessao_categorias_cookie(resp: HTMLResponse, sessao_id: str) -> HTMLResponse:
    resp.set_cookie(
        key=_COOKIE_CATEGORIAS_SESSAO,
        value=sessao_id,
        httponly=False,
        max_age=_SESSAO_MAX_AGE,
        samesite="lax",
...
def _hidratar_catalogo_categorias_form(sessao_id: str, catalogo_json: str = "") -> None:
    catalogo_categorias.hidratar_se_vazio(sessao_id, catalogo_json or "")


def _html_patch_catalogo_categorias(sessao_id: str) -> str:
    snap = catalogo_categorias.snapshot_sessao(sessao_id)
    blob = html.escape(json.dumps(snap, ensure_ascii=False, separators=(",", ":")))
    return (
        f'<textarea class="mercos-categorias-catalogo-blob" hidden readonly '
        f'aria-hidden="true">{blob}</textarea>'
    )


def _linhas_ciclo_categorias(
    sessao_id: str, estado: dict[str, Any] | None = None
) -> list[tuple[str, Any]]:
    estado = estado or catalogo_categorias.obter(sessao_id)
    ciclo = catalogo_categorias.obter_ciclo(sessao_id)
    sync = (estado or {}).get("ultima_sync") or {}
    tipo = sync.get("tipo")
    tipo_label = (
        "Completa"
    ...
    return [
        ...
        ("Requisições executadas", _num("requisicoes_executadas")),
        ("Total retornado em todas as páginas", sync.get("total_lote") or 0),
        ("Total no catálogo", len((estado or {}).get("categorias") or {})),
        ("Motivo da parada", sync.get("motivo_parada") or "—"),
        ("Status da sincronização", sync.get("status_sync") or "—"),
        ("Chamadas completas no ciclo", ciclo.get("chamadas_completas") or 0),
        ("Chamadas incrementais no ciclo", ciclo.get("chamadas_incrementais") or 0),
...
def _attrs_ciclo_categorias(sessao_id: str) -> dict[str, str]:
    ciclo = catalogo_categorias.obter_ciclo(sessao_id)
    return {
        "ciclo-ativo": "1" if ciclo.get("ativo") else "0",
        "etapa-interna": str(ciclo.get("etapa_interna") or 0),
...
def _html_tabela_categorias(itens: list) -> str:
    rows = []
    for item in itens or []:
        rows.append(
            [
                _campo(item, "id"),
                _campo(item, "nome", "name"),
                item.get("categoria_pai_id")
                if isinstance(item, dict)
                and item.get("categoria_pai_id") not in (None, "")
                else "—",
                _fmt_bool(_campo(item, "excluido", default=False)),
                item.get("ultima_alteracao")
                if isinstance(item, dict)
    ...
    return _table(
        ["ID", "Nome", "Categoria pai", "Excluído", "Última alteração"],
        rows,
    )


def _cursor_tabelas_preco(request: Request, cursor_form: str = "") -> str:
    form = (cursor_form or "").strip()
    if form:
        return form
    raw = (request.cookies.get(_COOKIE_TABELAS_PRECO_CURSOR) or "").strip().strip('"')
    if not raw:
        return ""
    try:
        return unquote(raw)
...
def _set_cursor_tabelas_preco_cookie(resp: HTMLResponse, cursor: str | None) -> HTMLResponse:
    valor = (cursor or "").strip()
    if valor:
        resp.set_cookie(
            key=_COOKIE_TABELAS_PRECO_CURSOR,
            value=quote(valor, safe=""),
            httponly=False,
            max_age=_CURSOR_MAX_AGE,
            samesite="lax",
        ...
        resp.delete_cookie(_COOKIE_TABELAS_PRECO_CURSOR)
    return resp


def _obter_ou_criar_sessao_tabelas_preco(request: Request) -> str:
    existente = (request.cookies.get(_COOKIE_TABELAS_PRECO_SESSAO) or "").strip()
    if existente:
        return existente
    return uuid.uuid4().hex


def _garantir_sessao_tabelas_preco_cookie(resp: HTMLResponse, sessao_id: str) -> HTMLResponse:
    resp.set_cookie(
        key=_COOKIE_TABELAS_PRECO_SESSAO,
        value=sessao_id,
        httponly=False,
        max_age=_SESSAO_MAX_AGE,
        samesite="lax",
...
def _hidratar_catalogo_tabelas_preco_form(sessao_id: str, catalogo_json: str = "") -> None:
    catalogo_tabelas_preco.hidratar_se_vazio(sessao_id, catalogo_json or "")


def _html_patch_catalogo_tabelas_preco(sessao_id: str) -> str:
    snap = catalogo_tabelas_preco.snapshot_sessao(sessao_id)
    blob = html.escape(json.dumps(snap, ensure_ascii=False, separators=(",", ":")))
    return (
        f'<textarea class="mercos-tabelas-preco-catalogo-blob" hidden readonly '
        f'aria-hidden="true">{blob}</textarea>'
    )


def _linhas_ciclo_tabelas_preco(
    sessao_id: str, estado: dict[str, Any] | None = None
) -> list[tuple[str, Any]]:
    estado = estado or catalogo_tabelas_preco.obter(sessao_id)
    ciclo = catalogo_tabelas_preco.obter_ciclo(sessao_id)
    sync = (estado or {}).get("ultima_sync") or {}
    tipo = sync.get("tipo")
    tipo_label = (
        "Completa"
    ...
    return [
        ...
        ("Requisições executadas", _num("requisicoes_executadas")),
        ("Total retornado em todas as páginas", sync.get("total_lote") or 0),
        ("Total no catálogo", len((estado or {}).get("tabelas") or {})),
        ("Motivo da parada", sync.get("motivo_parada") or "—"),
        ("Status da sincronização", sync.get("status_sync") or "—"),
        ("Chamadas completas no ciclo", ciclo.get("chamadas_completas") or 0),
        ("Chamadas incrementais no ciclo", ciclo.get("chamadas_incrementais") or 0),
...
def _attrs_ciclo_tabelas_preco(sessao_id: str) -> dict[str, str]:
    ciclo = catalogo_tabelas_preco.obter_ciclo(sessao_id)
    return {
        "ciclo-ativo": "1" if ciclo.get("ativo") else "0",
        "etapa-interna": str(ciclo.get("etapa_interna") or 0),
...
def _html_tabela_tabelas_preco(itens: list) -> str:
    rows = []
    for item in itens or []:
        rows.append(
            [
                _campo(item, "id"),
                _campo(item, "nome", "name"),
                _campo(item, "tipo", default="—"),
                _fmt_valor_minimo(item.get("acrescimo") if isinstance(item, dict) else None),
                _fmt_valor_minimo(item.get("desconto") if isinstance(item, dict) else None),
                _fmt_bool(_campo(item, "excluido", default=False)),
                item.get("ultima_alteracao")
                if isinstance(item, dict)
                and item.get("ultima_alteracao") not in (None, "")
    ...
    return _table(
        ["ID", "Nome", "Tipo", "Acréscimo", "Desconto", "Excluído", "Última alteração"],
        rows,
    )
...
def acao_categorias(request: Request, token: str = Form("")):
    _auth(request, token)
    bloqueio = _bloqueio_categorias_ciclo_ativo(request)
    ...
    try:
        ...
        rows = [
            [
                _campo(i, "id"),
                _campo(i, "nome"),
                _campo(i, "categoria_pai_id", default="—"),
                _fmt_bool(_campo(i, "excluido", default=False)),
            ]
            for i in (data.get("itens") or [])
        ]
        table = _table(["ID", "Nome", "Categoria pai", "Excluído"], rows)
...
def _bloqueio_categorias_ciclo_ativo(request: Request) -> HTMLResponse | None:
    """Bloqueia a busca simples antiga de Categorias durante o ciclo ativo."""
    sessao = (request.cookies.get(_COOKIE_CATEGORIAS_SESSAO) or "").strip()
    if not sessao or not catalogo_categorias.ciclo_ativo(sessao):
        return None
    resp = _wrap_result(
        _card(
            ...
            [
                (
                    "Mensagem",
                    "O ciclo de sincronização de categorias de produto está ativo. "
                    "Use apenas «Sincronizar próxima etapa» ou «Localizar categoria pelo nome».",
                )
            ]
            + _linhas_ciclo_categorias(sessao),
            status_label="Bloqueado",
            css="erro",
        ),
        extra_attrs={"status-sync": "bloqueado", **_attrs_ciclo_categorias(sessao)},
    )
    return _garantir_sessao_categorias_cookie(resp, sessao)


@router.post("/homologacao-ui/acoes/categorias-reiniciar", response_class=HTMLResponse)
def acao_categorias_reiniciar(
    request: Request,
    token: str = Form(""),
):
    """Apaga cursor/catálogo de categorias, ciclo volta a 0/3 — sem chamar a Mercos."""
    _auth(request, token)
    sessao = _obter_ou_criar_sessao_categorias(request)
    catalogo_categorias.iniciar_ciclo(sessao)
    homolog._limpar_resumes_da_sessao(sessao)
    estado = catalogo_categorias.obter(sessao)
    mensagem = (
        '<div class="result-card ok">'
        "<h4>Ciclo de sincronização reiniciado</h4>"
    ...
    resumo = _card(
        "Estado do ciclo",
        _linhas_ciclo_categorias(sessao, estado),
        status_label="Ciclo ativo",
        css="ok",
    )
    resp = _wrap_result(
        mensagem + resumo + _html_patch_catalogo_categorias(sessao),
        extra_attrs={
            "novo-cursor": "",
            "cursor-limpo": "1",
            "catalogo-limpo": "1",
            "status-sync": "reiniciado",
            "catalogo-total": "0",
            **_attrs_ciclo_categorias(sessao),
        },
    )
    resp = _set_cursor_categorias_cookie(resp, None)
    return _garantir_sessao_categorias_cookie(resp, sessao)


@router.post("/homologacao-ui/acoes/categorias-sincronizar", response_class=HTMLResponse)
def acao_categorias_sincronizar(
    request: Request,
    token: str = Form(""),
    cursor: str = Form(""),
    catalogo_json: str = Form(""),
    ...
    _auth(request, token)
    sessao = _obter_ou_criar_sessao_categorias(request)
    _hidratar_catalogo_categorias_form(sessao, catalogo_json)
    ciclo = catalogo_categorias.obter_ciclo(sessao)
    if not ciclo.get("ativo"):
        catalogo_categorias.iniciar_ciclo(sessao)
        ciclo = catalogo_categorias.obter_ciclo(sessao)

    etapa = int(ciclo.get("etapa_interna") or 0)
    cursor_form = _cursor_categorias(request, cursor)
    ...
    if etapa == 0:
        ...
        if not cursor_form:
            resp = _wrap_result(
                ...
                extra_attrs={"status-sync": "erro", **_attrs_ciclo_categorias(sessao)},
            )
            return _garantir_sessao_categorias_cookie(resp, sessao)
        cursor_para_sync = cursor_form
        tipo_esperado = "incremental"
    ...
    try:
        data = homolog.sincronizar_categorias(
        ...
        if tipo_real == "completa":
            catalogo_categorias.substituir_completo(
                sessao, data.get("itens") or [], meta=meta
            )
        else:
            catalogo_categorias.upsert_incremental(
                sessao, data.get("itens") or [], meta=meta
            )
        try:
            catalogo_categorias.registrar_sync_ciclo(sessao, tipo=tipo_real)
        except ValueError as exc:
            resp = _wrap_result(
                _card(
                    "Busca completa bloqueada durante a homologação",
                    [("Mensagem", str(exc))] + _linhas_ciclo_categorias(sessao),
                    status_label="Bloqueado",
                    css="erro",
                ),
                extra_attrs={"status-sync": "bloqueado", **_attrs_ciclo_categorias(sessao)},
        ...
        resumo = _card(
            "Sincronização de categorias de produto",
            [
        ...
        table = _html_tabela_categorias(data.get("itens") or [])
        resp = _wrap_result(
            resumo + table + _html_patch_catalogo_categorias(sessao),
            extra_attrs={
                "novo-cursor": data.get("novo_cursor") or "",
                "cursor-anterior": data.get("cursor_anterior") or "",
                "cursor-base": data.get("cursor_base") or "",
                ...
                "catalogo-total": str(len(estado.get("categorias") or {})),
                "catalogo-modo": "replace" if tipo_real == "completa" else "upsert",
                "status-sync": status_sync,
                **_attrs_ciclo_categorias(sessao),
            },
        )
        resp = _set_cursor_categorias_cookie(resp, data.get("novo_cursor"))
        return _garantir_sessao_categorias_cookie(resp, sessao)
    except MercosApiError as exc:
        if exc.status_code == 429:
            segundos = exc.retry_after if exc.retry_after is not None else 10
...
@router.post("/homologacao-ui/acoes/categorias-localizar", response_class=HTMLResponse)
def acao_categorias_localizar(
    request: Request,
    token: str = Form(""),
    nome: str = Form(""),
    catalogo_json: str = Form(""),
    ...
    sessao = _obter_ou_criar_sessao_categorias(request)
    _hidratar_catalogo_categorias_form(sessao, catalogo_json)
    ciclo_antes = catalogo_categorias.obter_ciclo(sessao)
    busca = (nome or "").strip()
    if not busca:
        return _garantir_sessao_categorias_cookie(
            _wrap_result(
                _card(
                    "Localizar categoria de produto",
                    [("Mensagem", "Informe o nome completo ou o prefixo da categoria.")],
                    status_label="Atenção",
                    css="erro",
                )
            ),
    ...
    encontrado, no_ultimo_lote, estado = catalogo_categorias.buscar_por_nome(
        sessao, busca
    )
    if not encontrado:
        return _garantir_sessao_categorias_cookie(
            _wrap_result(
                _card(
                    ...
                    [
                        ("Nome buscado", busca),
                        (
                            "Categorias no catálogo local",
                            catalogo_categorias.total(sessao),
                        ),
                    ]
                    + _linhas_ciclo_categorias(sessao, estado),
                    status_label="Não encontrado",
                ...
                extra_attrs={"cursor-fixo": "1", **_attrs_ciclo_categorias(sessao)},
            ),
            sessao,
        )
    ...
    if ultima is None or ultima == "":
        ultima = "—"
    pai = encontrado.get("categoria_pai_id")
    if pai is None or pai == "":
        pai = "—"
    linhas: list[tuple[str, Any]] = [
        ("ID", encontrado.get("id")),
    ...
    linhas.extend(_linhas_ciclo_categorias(sessao, estado))
    nota = ""
    if not no_ultimo_lote:
        nota = (
            '<p class="hint">Categoria localizada no catálogo do ERP; '
            "não veio no último lote incremental.</p>"
        )
    card = _card(
        "Categoria de produto localizada",
        linhas,
        status_label="Encontrado",
        css="ok",
    )
    ciclo_depois = catalogo_categorias.obter_ciclo(sessao)
    if ciclo_antes.get("etapa_interna") != ciclo_depois.get("etapa_interna"):
        catalogo_categorias._salvar_ciclo(sessao, ciclo_antes)
    resp = _wrap_result(
        nota + card + _html_patch_catalogo_categorias(sessao),
        extra_attrs={
            "status-sync": "localizado",
            "cursor-fixo": "1",
            **_attrs_ciclo_categorias(sessao),
...
@router.post(
    "/homologacao-ui/acoes/clientes-liberar-categorias",
    response_class=HTMLResponse,
)
def acao_clientes_liberar_categorias(
    request: Request,
    token: str = Form(""),
    cliente_id: str = Form(""),
    cnpj: str = Form(""),
    ...
    """POST /v1/clientes_categorias/liberar_todas — um único POST por clique.

    Libera TODAS as categorias de produto para o cliente. O cliente_id vai
    no corpo (contrato Apiary). Não envia lista de categorias; não cria nem
    altera cliente/categoria. Distinto do vínculo seletivo.
    """
    _auth(request, token)
    cid = (cliente_id or "").strip()
    cnpj_val = (cnpj or "").strip()
    ...
    try:
        out = homolog.liberar_todas_categorias_cliente(cid)
        status = out.get("status_code") or 200
        card = _card(
            "Categorias de produto liberadas",
            [
                ("Status HTTP", status),
                ("ID do cliente", cid),
                ("CNPJ", cnpj_val or "—"),
                ("Razão social", razao_val or "—"),
                ("Resultado", "Todas as categorias liberadas"),
                ("Origem", "Mercos Sandbox"),
            ],
            status_label=f"Status {status}",
            css="ok",
...
def acao_tabelas_preco(request: Request, token: str = Form("")):
    _auth(request, token)
    bloqueio = _bloqueio_tabelas_preco_ciclo_ativo(request)
    if bloqueio is not None:
        return bloqueio
    try:
...
def _bloqueio_tabelas_preco_ciclo_ativo(request: Request) -> HTMLResponse | None:
    """Bloqueia a busca simples antiga de Tabelas de Preço durante o ciclo ativo."""
    sessao = (request.cookies.get(_COOKIE_TABELAS_PRECO_SESSAO) or "").strip()
    if not sessao or not catalogo_tabelas_preco.ciclo_ativo(sessao):
        return None
    resp = _wrap_result(
        _card(
            "Busca manual bloqueada durante a homologação",
            [
                (
                    "Mensagem",
                    "O ciclo de sincronização de tabelas de preço está ativo. "
                    "Use apenas «Sincronizar próxima etapa» ou «Localizar tabela pelo nome».",
                )
            ]
            + _linhas_ciclo_tabelas_preco(sessao),
            status_label="Bloqueado",
            css="erro",
        ),
        extra_attrs={"status-sync": "bloqueado", **_attrs_ciclo_tabelas_preco(sessao)},
    )
    return _garantir_sessao_tabelas_preco_cookie(resp, sessao)


@router.post("/homologacao-ui/acoes/tabelas-preco-reiniciar", response_class=HTMLResponse)
def acao_tabelas_preco_reiniciar(
    request: Request,
    token: str = Form(""),
):
    """Apaga cursor/catálogo de tabelas de preço, ciclo volta a 0/3 — sem chamar a Mercos."""
    _auth(request, token)
    sessao = _obter_ou_criar_sessao_tabelas_preco(request)
    catalogo_tabelas_preco.iniciar_ciclo(sessao)
    homolog._limpar_resumes_da_sessao(sessao)
    estado = catalogo_tabelas_preco.obter(sessao)
    mensagem = (
        '<div class="result-card ok">'
        "<h4>Ciclo de sincronização reiniciado</h4>"
        "<p>Cursor e catálogo anteriores apagados. Novo ciclo ativo na etapa 0/3. "
    ...
    resumo = _card(
        "Estado do ciclo",
        _linhas_ciclo_tabelas_preco(sessao, estado),
        status_label="Ciclo ativo",
        css="ok",
    )
    resp = _wrap_result(
        mensagem + resumo + _html_patch_catalogo_tabelas_preco(sessao),
        extra_attrs={
            "novo-cursor": "",
            "cursor-limpo": "1",
            "catalogo-limpo": "1",
            "status-sync": "reiniciado",
            "catalogo-total": "0",
            **_attrs_ciclo_tabelas_preco(sessao),
        },
    )
    resp = _set_cursor_tabelas_preco_cookie(resp, None)
    return _garantir_sessao_tabelas_preco_cookie(resp, sessao)


@router.post("/homologacao-ui/acoes/tabelas-preco-sincronizar", response_class=HTMLResponse)
def acao_tabelas_preco_sincronizar(
    request: Request,
    token: str = Form(""),
    cursor: str = Form(""),
    catalogo_json: str = Form(""),
    ...
    _auth(request, token)
    sessao = _obter_ou_criar_sessao_tabelas_preco(request)
    _hidratar_catalogo_tabelas_preco_form(sessao, catalogo_json)
    ciclo = catalogo_tabelas_preco.obter_ciclo(sessao)
    if not ciclo.get("ativo"):
        catalogo_tabelas_preco.iniciar_ciclo(sessao)
        ciclo = catalogo_tabelas_preco.obter_ciclo(sessao)

    etapa = int(ciclo.get("etapa_interna") or 0)
    cursor_form = _cursor_tabelas_preco(request, cursor)
    ...
    if etapa == 0:
        ...
        if not cursor_form:
            resp = _wrap_result(
                ...
                extra_attrs={"status-sync": "erro", **_attrs_ciclo_tabelas_preco(sessao)},
            )
            return _garantir_sessao_tabelas_preco_cookie(resp, sessao)
        cursor_para_sync = cursor_form
        tipo_esperado = "incremental"
    ...
    try:
        data = homolog.sincronizar_tabelas_preco(
            cursor_para_sync, max_paginas=20, sessao_id=sessao
        )
        tipo_real = data.get("tipo") or tipo_esperado
        if etapa >= 1 and (
        ...
        if tipo_real == "completa":
            catalogo_tabelas_preco.substituir_completo(
                sessao, data.get("itens") or [], meta=meta
            )
        else:
            catalogo_tabelas_preco.upsert_incremental(
                sessao, data.get("itens") or [], meta=meta
            )
        try:
            catalogo_tabelas_preco.registrar_sync_ciclo(sessao, tipo=tipo_real)
        except ValueError as exc:
            resp = _wrap_result(
                _card(
                    "Busca completa bloqueada durante a homologação",
                    [("Mensagem", str(exc))] + _linhas_ciclo_tabelas_preco(sessao),
                    status_label="Bloqueado",
                    css="erro",
                ),
                extra_attrs={"status-sync": "bloqueado", **_attrs_ciclo_tabelas_preco(sessao)},
        ...
        table = _html_tabela_tabelas_preco(data.get("itens") or [])
        resp = _wrap_result(
            resumo + table + _html_patch_catalogo_tabelas_preco(sessao),
            extra_attrs={
                "novo-cursor": data.get("novo_cursor") or "",
                "cursor-anterior": data.get("cursor_anterior") or "",
                "cursor-base": data.get("cursor_base") or "",
                ...
                "requisicoes-executadas": str(data.get("requisicoes_executadas") or 0),
                "motivo-parada": motivo,
                "catalogo-total": str(len(estado.get("tabelas") or {})),
                "catalogo-modo": "replace" if tipo_real == "completa" else "upsert",
                "status-sync": status_sync,
                **_attrs_ciclo_tabelas_preco(sessao),
            },
        )
        resp = _set_cursor_tabelas_preco_cookie(resp, data.get("novo_cursor"))
        return _garantir_sessao_tabelas_preco_cookie(resp, sessao)
    except MercosApiError as exc:
        if exc.status_code == 429:
            segundos = exc.retry_after if exc.retry_after is not None else 10
...
@router.post("/homologacao-ui/acoes/tabelas-preco-localizar", response_class=HTMLResponse)
def acao_tabelas_preco_localizar(
    request: Request,
    token: str = Form(""),
    nome: str = Form(""),
    catalogo_json: str = Form(""),
    ...
    sessao = _obter_ou_criar_sessao_tabelas_preco(request)
    _hidratar_catalogo_tabelas_preco_form(sessao, catalogo_json)
    ciclo_antes = catalogo_tabelas_preco.obter_ciclo(sessao)
    busca = (nome or "").strip()
    if not busca:
        return _garantir_sessao_tabelas_preco_cookie(
            _wrap_result(
                _card(
                    "Localizar tabela de preço",
                    [("Mensagem", "Informe o nome completo ou o prefixo da tabela.")],
                    status_label="Atenção",
                    css="erro",
                )
            ),
    ...
    encontrado, no_ultimo_lote, estado = catalogo_tabelas_preco.buscar_por_nome(
        sessao, busca
    )
    if not encontrado:
        return _garantir_sessao_tabelas_preco_cookie(
            _wrap_result(
                _card(
                    ...
                    [
                        ("Nome buscado", busca),
                        (
                            "Tabelas no catálogo local",
                            catalogo_tabelas_preco.total(sessao),
                        ),
                    ]
                    + _linhas_ciclo_tabelas_preco(sessao, estado),
                    status_label="Não encontrado",
                ...
                extra_attrs={"cursor-fixo": "1", **_attrs_ciclo_tabelas_preco(sessao)},
            ),
            sessao,
        )
    ...
    if tipo is None or tipo == "":
        tipo = "—"
    acrescimo = encontrado.get("acrescimo")
    if acrescimo is None or acrescimo == "":
        acrescimo = "—"
    else:
        try:
            acrescimo = f"{float(acrescimo):.2f}"
        except (TypeError, ValueError):
            acrescimo = str(acrescimo)
    desconto = encontrado.get("desconto")
    if desconto is None or desconto == "":
        desconto = "—"
    else:
        try:
            desconto = f"{float(desconto):.2f}"
        except (TypeError, ValueError):
    ...
    linhas.extend(_linhas_ciclo_tabelas_preco(sessao, estado))
    nota = ""
    if not no_ultimo_lote:
        nota = (
            '<p class="hint">Tabela localizada no catálogo do ERP; '
            "não veio no último lote incremental.</p>"
        )
    card = _card(
        "Tabela de preço localizada",
        linhas,
        status_label="Encontrado",
        css="ok",
    )
    ciclo_depois = catalogo_tabelas_preco.obter_ciclo(sessao)
    if ciclo_antes.get("etapa_interna") != ciclo_depois.get("etapa_interna"):
        catalogo_tabelas_preco._salvar_ciclo(sessao, ciclo_antes)
    resp = _wrap_result(
        nota + card + _html_patch_catalogo_tabelas_preco(sessao),
        extra_attrs={
            "status-sync": "localizado",
            "cursor-fixo": "1",
            **_attrs_ciclo_tabelas_preco(sessao),
