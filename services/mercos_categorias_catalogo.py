"""Catálogo local acumulado de categorias de produto para homologação Mercos.

Memória por sessão; não chama a API Mercos. Usado pela UI de sincronização e
localização de Categoria de Produto GET. Entidade distinta de Produto GET.
Ciclo de 3 etapas. Espelha services/mercos_tipos_pedido_catalogo.py.
"""
...
def _normalizar_categoria(item: dict) -> dict[str, Any] | None:
    if not isinstance(item, dict):
        return None
    cid = item.get("id")
    if cid is None or cid == "":
        return None
    # Campos confirmados no diagnóstico sandbox 2026-07-20 + Apiary.
    out: dict[str, Any] = {
        "id": cid,
        "nome": item.get("nome") or item.get("name"),
        "categoria_pai_id": item.get("categoria_pai_id"),
...
def substituir_completo(
    ...
    *,
    ...
    for item in itens or []:
        norm = _normalizar_categoria(item) if isinstance(item, dict) else None
        if not norm:
            continue
        chave = str(norm["id"])
        categorias[chave] = norm
...
def hidratar_se_vazio(sessao_id: str, payload: Any) -> dict[str, Any]:
    ...
    if not atual.get("categorias") and itens:
...
def buscar_por_nome(
    ...
    exato = None
    por_prefixo = None
    for cat in (estado.get("categorias") or {}).values():
        if not isinstance(cat, dict):
            continue
        nome_cat = str(cat.get("nome") or "")
        if nome_cat == busca or nome_cat.lower() == busca_l:
            exato = cat
            break
        if por_prefixo is None and nome_cat.lower().startswith(busca_l):
            por_prefixo = cat
...
def snapshot_sessao(sessao_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "total": len(estado.get("categorias") or {}),
    }


def total(sessao_id: str) -> int:
    return len(obter(sessao_id).get("categorias") or {})
