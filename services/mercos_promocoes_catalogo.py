"""Catálogo local acumulado de promoções para homologação Mercos.

Memória por sessão; não chama a API Mercos. Usado pela UI de sincronização e
localização de Promoções GET. Entidade distinta de produto, tabela preço, cupom
e condição de pagamento. Ciclo de 2 etapas.
"""
...
def _normalizar_promocao(item: dict) -> dict[str, Any] | None:
    ...
    if item.get("representada_id") not in (None, ""):
        out["representada_id"] = item.get("representada_id")
    return out
...
def substituir_completo(
    ...
    *,
    ...
    for item in itens or []:
        norm = _normalizar_promocao(item) if isinstance(item, dict) else None
        if not norm:
            continue
        chave = str(norm["id"])
        promocoes[chave] = norm
...
def hidratar_se_vazio(sessao_id: str, payload: Any) -> dict[str, Any]:
    ...
    if not atual.get("promocoes") and itens:
...
def snapshot_sessao(sessao_id: str) -> dict[str, Any]:
    estado = obter(sessao_id)
    return {
        "promocoes": estado.get("promocoes") or {},
        ...
        "total": len(estado.get("promocoes") or {}),
    }


def total(sessao_id: str) -> int:
    return len(obter(sessao_id).get("promocoes") or {})
