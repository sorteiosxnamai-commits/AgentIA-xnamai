"""Catálogo local acumulado de tabelas de preço para homologação Mercos.

Memória por sessão; não chama a API Mercos. Usado pela UI de sincronização e
localização de Tabela de Preço GET. Entidade distinta de Tabela POST e de
preço por produto. Ciclo de 3 etapas. Espelha services/mercos_categorias_catalogo.py.
"""
...
def _normalizar_tabela(item: dict) -> dict[str, Any] | None:
...
def substituir_completo(
    ...
    *,
    ...
    for item in itens or []:
        norm = _normalizar_tabela(item) if isinstance(item, dict) else None
        if not norm:
            continue
        chave = str(norm["id"])
        tabelas[chave] = norm
...
def hidratar_se_vazio(sessao_id: str, payload: Any) -> dict[str, Any]:
    ...
    if not atual.get("tabelas") and itens:
...
def buscar_por_nome(
    ...
    exato = None
    por_prefixo = None
    for tab in (estado.get("tabelas") or {}).values():
        if not isinstance(tab, dict):
            continue
        nome_tab = str(tab.get("nome") or "")
        if nome_tab == busca or nome_tab.lower() == busca_l:
            exato = tab
            break
        if por_prefixo is None and nome_tab.lower().startswith(busca_l):
            por_prefixo = tab
...
def snapshot_sessao(sessao_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "total": len(estado.get("tabelas") or {}),
    }


def total(sessao_id: str) -> int:
    return len(obter(sessao_id).get("tabelas") or {})
