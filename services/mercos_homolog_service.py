...
def inventario_homologacao() -> dict[str, Any]:
    ...
    return {
        ...
        "entidades": [
            ...
            {"entidade": "Faturamento de Pedido", "metodo": "PUT", "path": _path("faturamento") + "/{id}", "status": "pronto"},
...
def listar_condicoes_pagamento_paginado_seguro(
    *,
    alterado_apos: str | None = None,
    pagina_inicial: int = 1,
    max_paginas: int = CLIENTES_MAX_PAGINAS_SYNC,
    ...
    """Lista condições de pagamento com o contrato seguro de tipos de pedido.

    Diagnóstico sandbox 2026-07-19 (3 chamadas): GET /v1/condicoes_pagamento
    responde lotes de 2 itens com os headers MEUSPEDIDOS_LIMITOU_REGISTROS /
    QTDE_TOTAL_REGISTROS / REQUISICOES_EXTRAS (total 5 → extras 2 → 3
    chamadas na completa). ``alterado_apos`` é keyset estritamente maior
    sobre ``ultima_alteracao`` (cursor exato, sem overlap). Campos: id, nome,
    valor_minimo, disponivel_b2b, excluido, ultima_alteracao. Excluídos vêm
    no payload com a flag ``excluido``.
    """
    return _listar_paginado_seguro(
        path=_path("condicoes_pagamento"),
        resume_ns="condicoes_pagamento",
        alterado_apos=alterado_apos,
        pagina_inicial=pagina_inicial,
        max_paginas=max_paginas,
        timeout_request=timeout_request,
...
_SYNC_CONDICOES_PAGAMENTO_LOCK = _ExpiringLock(ttl_seconds=CLIENTES_LOCK_TTL_SEGUNDOS)


def sincronizar_condicoes_pagamento(
    cursor: str | None = None,
    *,
    max_paginas: int = CLIENTES_MAX_PAGINAS_SYNC,
    timeout_request: float = CLIENTES_TIMEOUT_REQUEST_SEGUNDOS,
    ...
    """Uma sincronização Condição de Pagamento GET (ciclo de 3 etapas).

    Busca completa (sem cursor) percorre TODOS os lotes: executa exatamente
    1 + MEUSPEDIDOS_REQUISICOES_EXTRAS chamadas quando o header existir, sem
    chamada final esperando lote vazio. Incremental envia o cursor EXATO da
    sincronização anterior (alterado_apos estritamente maior, confirmado no
    diagnóstico). Respeita Retry-After/backoff por página e libera o lock
    sempre no finally.
    """
    if not _SYNC_CONDICOES_PAGAMENTO_LOCK.acquire(blocking=False):
        raise MercosApiError(
            "Sincronização de condições de pagamento já em andamento. Aguarde a conclusão.",
            status_code=409,
        )
    try:
        cursor_base = (cursor or "").strip() or None
        ...
        # Cursor exato: enviado byte a byte como foi salvo.
        alterado_apos = cursor_base
        data = listar_condicoes_pagamento_paginado_seguro(
            alterado_apos=alterado_apos,
            pagina_inicial=1,
            max_paginas=max_paginas,
            timeout_request=timeout_request,
...
def listar_categorias_paginado_seguro(
    *,
    alterado_apos: str | None = None,
    pagina_inicial: int = 1,
    max_paginas: int = CLIENTES_MAX_PAGINAS_SYNC,
    ...
    """Lista categorias de produto com o contrato seguro de tipos/condições.

    Diagnóstico sandbox 2026-07-20 (4 chamadas): GET /v1/categorias responde
    lotes de 2 itens com headers MEUSPEDIDOS_LIMITOU_REGISTROS /
    QTDE_TOTAL_REGISTROS / REQUISICOES_EXTRAS (total 9 → extras 4 → 5
    chamadas na completa). ``alterado_apos`` é keyset estritamente maior
    sobre ``ultima_alteracao`` (cursor exato). Campos: id, nome,
    categoria_pai_id (quando houver), excluido, ultima_alteracao.
    """
    return _listar_paginado_seguro(
        path=_path("categorias"),
        resume_ns="categorias",
        alterado_apos=alterado_apos,
        pagina_inicial=pagina_inicial,
        max_paginas=max_paginas,
        timeout_request=timeout_request,
...
_SYNC_CATEGORIAS_LOCK = _ExpiringLock(ttl_seconds=CLIENTES_LOCK_TTL_SEGUNDOS)
...
def sincronizar_categorias(
    ...
    *,
    ...
    """Uma sincronização Categoria de Produto GET (ciclo de 3 etapas).

    Busca completa percorre todos os lotes (1 + extras). Incremental envia
    o cursor EXATO. Lock próprio liberado no finally.
    """
    if not _SYNC_CATEGORIAS_LOCK.acquire(blocking=False):
        raise MercosApiError(
            "Sincronização de categorias já em andamento. Aguarde a conclusão.",
            status_code=409,
        )
    try:
        cursor_base = (cursor or "").strip() or None
        tipo = "incremental" if cursor_base else "completa"
        alterado_apos = cursor_base
        data = listar_categorias_paginado_seguro(
            alterado_apos=alterado_apos,
            pagina_inicial=1,
            max_paginas=max_paginas,
            timeout_request=timeout_request,
...
def criar_condicao_pagamento(body: dict) -> dict:
    """POST /v1/condicoes_pagamento — um único envio à Mercos.

    Contrato oficial (Apiary): nome (obrigatório, String:100); opcionais
    valor_minimo (Double), disponivel_b2b (Boolean),
    considerar_limite_credito (Boolean), excluido (Boolean). Sucesso 201;
    validação 412. ID criado no header MeusPedidosID (capturado por
    post_json). Entidade distinta de Formas de Pagamento / Pagamentos /
    Faturamento. Não altera o ciclo GET.
    """
    payload = dict(body or {})
    payload.pop("id", None)
    nome = str(payload.get("nome") or "").strip()
...
def alterar_faturamento(
    faturamento_id: int | str,
    pedido_id: int | str,
    valor_faturado: float,
    *,
    data_faturamento: str | None = None,
) -> dict:
    """PUT /v1/faturamento/{id} — id do FATURAMENTO na URL (não o do pedido).

    Contrato oficial (Apiary): corpo objeto com pedido_id, valor_faturado e
    data_faturamento obrigatórios (mesma tabela do POST). Sucesso 201; erros
    412/404. Não executa POST novamente nem o PUT comum de pedidos.
    """
    fid = str(faturamento_id or "").strip()
    if not fid:
        raise MercosApiError(
            "Informe o ID do faturamento para alteração.", status_code=422
        )
    pid = str(pedido_id or "").strip()
    if not pid:
        raise MercosApiError(
            "Informe o ID Mercos do pedido do faturamento.", status_code=422
        )
    try:
        valor = round(float(valor_faturado), 2)
    except (TypeError, ValueError):
    ...
    return put_json(f"{_path('faturamento')}/{fid}", body)
