"""Catálogo local acumulado de pedidos para homologação Mercos (memória por sessão).

Não chama a API Mercos. Usado pela UI de sincronização/localização de pedidos.
Espelha services/mercos_usuarios_catalogo.py, com ciclo de 2 etapas (Pedido GET).
"""
...
def _normalizar_pedido(item: dict) -> dict[str, Any] | None:
...
def substituir_completo(
    ...
    *,
    ...
    for item in itens or []:
        norm = _normalizar_pedido(item) if isinstance(item, dict) else None
        if not norm:
            continue
        chave = str(norm["id"])
        pedidos[chave] = norm
...
def hidratar_se_vazio(sessao_id: str, payload: Any) -> dict[str, Any]:
    ...
    if not atual.get("pedidos") and itens:
...
def _razao_do_pedido(
    pedido: dict[str, Any],
    razoes_por_cliente_id: dict[str, str] | None,
) -> str:
    razao = str(pedido.get("cliente_razao_social") or "")
    if razao:
        return razao
    if razoes_por_cliente_id:
        return str(razoes_por_cliente_id.get(str(pedido.get("cliente_id"))) or "")
    return ""
...
def buscar_por_razao_social(
    ...
    razao_social: str,
    *,
    razoes_por_cliente_id: dict[str, str] | None = None,
) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    """Busca pedido no catálogo local pela razão social do cliente (exata ou prefixo).

    Não chama a Mercos. Se o pedido não trouxer a razão social, relaciona o
    cliente_id com o catálogo local de clientes (razoes_por_cliente_id).
    """
    ...
    for ped in (estado.get("pedidos") or {}).values():
        if not isinstance(ped, dict):
            continue
        razao = _razao_do_pedido(ped, razoes_por_cliente_id)
        if not razao:
            continue
        if razao == busca or razao.lower() == busca_l:
            exato = dict(ped, cliente_razao_social=razao)
            break
        if por_prefixo is None and razao.lower().startswith(busca_l):
            por_prefixo = dict(ped, cliente_razao_social=razao)
    encontrado = exato or por_prefixo
    if not encontrado:
        return None, estado
    return deepcopy(encontrado), estado
...
def snapshot_sessao(sessao_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "total": len(estado.get("pedidos") or {}),
    }


def total(sessao_id: str) -> int:
    return len(obter(sessao_id).get("pedidos") or {})
