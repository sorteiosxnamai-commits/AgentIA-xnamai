"""Catálogo local acumulado de tipos de pedido para homologação Mercos.

Memória por sessão; não chama a API Mercos. Usado pela UI de sincronização e
localização de tipos de pedido. Espelha services/mercos_usuarios_catalogo.py.
"""
...
def _normalizar_tipo(item: dict) -> dict[str, Any] | None:
...
def substituir_completo(
    ...
    *,
    ...
    for item in itens or []:
        norm = _normalizar_tipo(item) if isinstance(item, dict) else None
        if not norm:
            continue
        chave = str(norm["id"])
        tipos[chave] = norm
...
def hidratar_se_vazio(sessao_id: str, payload: Any) -> dict[str, Any]:
    ...
    if not atual.get("tipos") and itens:
...
def buscar_por_nome(
    ...
    exato = None
    por_prefixo = None
    for tp in (estado.get("tipos") or {}).values():
        if not isinstance(tp, dict):
            continue
        nome_tp = str(tp.get("nome") or "")
        if nome_tp == busca or nome_tp.lower() == busca_l:
            exato = tp
            break
        if por_prefixo is None and nome_tp.lower().startswith(busca_l):
            por_prefixo = tp
...
def snapshot_sessao(sessao_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "total": len(estado.get("tipos") or {}),
    }


def total(sessao_id: str) -> int:
    return len(obter(sessao_id).get("tipos") or {})
